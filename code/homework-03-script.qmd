...
#read in data here
salinity <- pickleweed <- read_csv(here::here("data", "salinity-pickleweed.csv"))
...
The assumptions I checked for a Pearson correlation model began with a Q-Q Residuals plot assesing normally distributed residuals using a QQ plot, here it shows the quantiles following a straight line of the theoretical normal quantiles with only a slight deviation at the end, proving to be normally distributed residuals. The Residuals vs Fitted plot checks for homoscedasticity (constant variance) which is proven due to the residuals being somewhat random and evenly spread around the horizontal dotted line, the Scale-Location plot assesses the same but uses the square root of the standardized residuals and this plot also proved there to be constant variance. Lastly in Residuals vs Leverage, this plot determines if there are any outliers influencing the model with Cook's distance, since there aren't any points outside the dotted lines this means there aren't any outliers that would influence the model, which meets all the assumptions for Pearson's correlation.
